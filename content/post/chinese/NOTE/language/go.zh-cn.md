...
go run xx.go # 运行
go get github.com/xx/xxx # 获取外部包
...
```go
append(slice, element1, element2...) //返回一个新的切片，赋值给原始切片来更新，添加到切片末尾
append(slice, slice...) //合并两个切片，修改函数内的局部变量而非外部传入的原始切片。
```
...
- 使用`panic/recover`模式处理错误，`recover`只在`defer`调用的函数中有效，panic将会异常退出
...
```go
fmt.Printf("%T", str)
// %b:二进制, %o:八进制, %x:十六进制, %#v:结构体名+结构体, %.2f:2位浮点数, %p:地址值, %v:数据值, %s:字符串
```
...
```go
copy( destSlice, srcSlice []T) int	//返回的int为实际发生复制的元素个数
```
