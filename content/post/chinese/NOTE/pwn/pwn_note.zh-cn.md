...
prefix更改为了`ctrl + a`，默认为`ctrl + b`，`gdb.attach`先要打开tmux
...
1. 按`prefix`+ `Ctrl`+`c`键，buffer缓存的内容将通过`xlip`程序复制到粘贴板
2. 按`prefix`+ `Ctrl`+`v`键，tmux将通过`xclip`访问粘贴板命令，然后由set-buffer设置给buffer缓存，最后由paste-buffer粘贴到tmux会话中。
...
- 针对代码段（.text）、数据段（.data）、未初始化全局变量段（.bss）等固定地址的一个防护技术
...
> 有canary栈溢出无法执行，在函数入口处从fs(32位)/gs(64位)寄存器读取一个值，IDA中可分析识别出canary
...
- 加载器将这些短从ELF文件复制到响应虚拟内存地址，通过页表建立虚拟和物理内存地址映射关系
...
- 低版本libc可以通过该泄露出的地址进行操作
- canary检查不符合的时候引起程序终止时，会打印程序名，而程序名作为`arg[0]`，存在于`stack`上
...
- read函数后的使得rbp进入`bss`段中，接着返回read函数前
...
4. 已知后门函数距起始位置地址0xabcd，则覆盖返回地址时只覆盖最低**2**字节，有**1/16**概率爆破成功
...
- **type**
  ...
  - `s`：输出null结尾的字符串直到精度规定上限，将栈中的值以地址进行解析，输出该值（作为指针）指向的字符串内容（存于数据段），若该值不能解析为地址则程序崩溃
...
- 有多次利用机会，目标为覆盖返回地址为后门地址

```python
rbp     8  0x7fffffffdfe0
		9  0x7fffffffdfe8 —▸ 0x4012ba (main+28) ◂— mov    eax, 0 # 返回地址
		   ...
