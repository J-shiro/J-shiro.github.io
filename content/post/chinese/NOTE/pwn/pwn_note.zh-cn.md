...
- 数据段 Data Segment 包含可读可写数据
  ...
  - `.got`节
  - `.got.plt`节
  - `.bss`节：（Block Started by Symbol）未初始化的全局变量，不占用实际内存空间
- 栈段 Stack Segment
  - 局部变量，指针
...
rbp     8  0x7fffffffdfe0
		9  0x7fffffffdfe8 —▸ 0x4012ba (main+28) ◂— mov    eax, 0 # 返回地址
		   ...
        15 0x7fffffffe018 —▸ 0x7fffffffe118 —▸ 0x7fffffffe437 ◂— '/home/pwn'
           ...
...
# f'%{func & 0xffff}c%number$hn' # 将func地址作为数值(16位)作为%c写入到栈中第number个参数指向的地址中
```
